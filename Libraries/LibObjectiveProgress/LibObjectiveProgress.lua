...
-- Weak-keyed cache; entries are collected by the GC if memory is needed
...
function LOP:GetNPCWeightByMap(mapID, npcID, isTeeming, isAlternate)
	...
	if not LOP.MapBasedWeights then
	...
	end -- Allows alternate maps for weird scenarios
	...
	if
		not LOP.MapBasedWeights[mapID]
		or not LOP.MapBasedWeights[mapID][tableIndex]
		or not LOP.MapBasedWeights[mapID][tableIndex][npcID]
	then
		return nil
	end
...
function LOP:GetNPCWeightByQuest(questID, npcID)
	-- Load map-based weight data if needed
	if not LOP.QuestBasedWeights then
		LOP:LoadWeightDataByQuest()
	end

	-- Ensure that specified map / NPC is valid
	if not LOP.QuestBasedWeights[questID] or not LOP.QuestBasedWeights[questID][npcID] then
		return nil
	end
...
function LOP:GetNPCWeightByCurrentQuests(npcID)
	...
	for questLogIndex = 1, numEntries do
		...
		if questInfo and not questInfo.isHeader and questInfo.questID ~= 0 then
			...
			if weight then
				questTable = questTable or acquireTable()
