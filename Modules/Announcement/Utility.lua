...
local function TryAnnounce(spellId, sourceName, id, list, spellType)
    ...
    local spellConfig = (spellType and A.db.utility.spells[spellType]) or (id and A.db.utility.spells[tostring(id)])
    if not spellConfig or not channelConfig then
        return
    end

    if (id and spellId == id) or (spellType and list[spellId]) then
