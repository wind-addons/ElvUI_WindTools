...
local CR = E:NewModule("Wind_CorruptionRank", "AceEvent-3.0", "AceHook-3.0")
...
local linkCache = {}
function CR:Corruption_Search(link)
...
local function getIconString(icon) return format("|T%s:14:14:0:0:64:64:5:59:5:59|t ", icon) end

function CR:Corruption_Convert(name, icon, level)
...
function CR:Corruption_UpdateSpell(value)
    if not value.name or not value.icon then value.name, _, value.icon = GetSpellInfo(value.spellID) end
end

function CR:Corruption_Update()
    local link = select(2, self:GetItem())
    if link and IsCorruptedItem(link) then
        local value = CR:Corruption_Search(link)
        if value then
            CR:Corruption_UpdateSpell(value)
            CR.Corruption_Convert(self, value.name, value.icon, value.level)
        end
    end
end
...
local summaries = {}
function CR:Corruption_Summary(unit)
    wipe(summaries)
...
function CR:Corruption_AddSummary()
    GameTooltip:AddLine(" ")
    for value, count in next, summaries do
        GameTooltip:AddLine(count .. " " .. getIconString(value.icon) .. value.name .. " " .. value.level, corruptionR,
                            corruptionG, corruptionB)
    end
    if not next(summaries) then GameTooltip:AddLine(NONE, corruptionR, corruptionG, corruptionB) end
    GameTooltip:Show()
end

function CR:Corruption_PlayerSummary()
    CR:Corruption_Summary("player")
...
function CR:Corruption_SearchCloak()
    ...
    for i = 1, tip:NumLines() do
        ...
        if value then resistance = value end
    end
    return resistance
end

function CR:Corruption_InspectSummary()
    ...
    GameTooltip:AddDoubleLine(TOTAL_CORRUPTION_TOOLTIP_LINE, max((total - resistance), 0), corruptionR, corruptionG,
                              corruptionB, corruptionR, corruptionG, corruptionB)
    CR:Corruption_AddSummary()
end
...
function CR:Initialize()
    if not E.db.WindTools["Trade"]["Corruption Rank"].enabled then return end
    if IsAddOnLoaded("CorruptionTooltips") then return end
    if IsAddOnLoaded("CorruptionNameTooltips") then return end

    self.db = E.db.WindTools["Trade"]["Corruption Rank"]
    tinsert(WT.UpdateAll, function() CR.db = E.db.WindTools["Trade"]["Corruption Rank"] end)

    GameTooltip:HookScript("OnTooltipSetItem", CR.Corruption_Update)
    ItemRefTooltip:HookScript("OnTooltipSetItem", CR.Corruption_Update)
    ShoppingTooltip1:HookScript("OnTooltipSetItem", CR.Corruption_Update)
    ShoppingTooltip2:HookScript("OnTooltipSetItem", CR.Corruption_Update)
    EmbeddedItemTooltip:HookScript("OnTooltipSetItem", CR.Corruption_Update)

    hooksecurefunc("CharacterFrameCorruption_OnEnter", CR.Corruption_PlayerSummary)
    CharacterStatsPane.ItemLevelFrame.Corruption:HookScript("OnEnter", CR.Corruption_PlayerSummary)

    self:RegisterEvent("INSPECT_READY")
end

local function InitializeCallback() CR:Initialize() end

E:RegisterModule(CR:GetName(), InitializeCallback)
