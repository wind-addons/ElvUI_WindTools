...
3. **聊天文字**: 更加全面的频道缩写，替换角色定位图标，去除方括号，去除服务器等
4. **右键菜单**: 一个安全无污染的右键菜单，支持邀请加入公会，好友申请，英雄榜等
...
3. **静音**: 自定义停用一些恼人的音效，如飞机头，武僧职业坐骑
4. **顿号转斜杠**: 中文输入法依旧可以进行命令输入，输入命令快人一步
5. **关闭特写**: 强制关闭特写框体
6. **标签**: 各职业颜色，目标推测距离，无符号血量，无符号能量，等等
7. **游戏条**: 添加一个美观且实用的游戏条
8. **跳过过场动画**: 跳过大部分过场动画; 按下修饰键可暂时停用
9. **预组列表**: 美化预组建队伍列表的图标，并添加一个职业颜色条
10. **移动速度**: 在人物面板显示移动速度
11. **自动化**: 自动接受复活，召唤及在战斗中隐藏界面
12. **无汉字计数**: 在数值中使用英文字符代替汉字
13. **自动截图**: 在你获得成就后自动进行截图
14. **隐藏制造者**: 隐藏物品鼠标提示中的制造者姓名
15. **键位在冷却上方**: 调整层级关系，让按键显示于 ElvUI 冷却动画上方
16. **公会新闻装等**: 在公会新闻中显示每件装备的物品等级
