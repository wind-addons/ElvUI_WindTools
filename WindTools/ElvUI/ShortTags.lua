...
local function ShortValue(number, noDecimal)
	...
	elseif E.db.general.numberPrefixStyle == "CHINESE" then
		if abs(number) >= 1e8 then
			return format("%.1f%s", number / 1e8, WindLocale["Y"])
		elseif abs(number) >= 1e4 then
			return format("%.1f%s", number / 1e4, WindLocale["W"])
		else
			return format("%d", number)
		end
	else
...
ElvUF.Tags.Methods["health:percent-short"] = function(unit)
	...
	if UnitIsDead(unit) then
		String = WindLocale["Dead"]
	elseif UnitIsGhost(unit) then
		String = WindLocale["Ghost"]
	elseif not UnitIsConnected(unit) then
		String = WindLocale["Offline"]
	else
